		...
		<div class="header-right">
			...
			<form action="/MyInternetShop/FrontController" method="post">
				<input type="hidden" id="command" name="command"
					...
					name="pageName" value="common_pages/register.jsp"> <select id="language"
			...
			<a class="active" href="register.jsp"><fmt:message
	...
	<div class="container">
		<div class="row content">
			...
			<div class="col-md-6">
				...
				<!-- you also need to add special character before(same as one in the first line of this page include file="components/message.jsp"% -->
				<jsp:include page="../message.jsp" />

				<form action="/MyInternetShop/FrontController" method="post"
					style="border: 1px solid #ccc" autocomplete="off">
					<input type="hidden" id="command" name="command" value="REGISTER">
					
					<div class="form-group">
						<label for="text"><fmt:message key="label.name" /></label> <input
							type="text" placeholder="<fmt:message key="label.entername" />"
							name="user_first_name" class="form-control" required> 
					</div>
					
					<div class="form-group">
						<label for="text"><fmt:message key="label.surname" /></label> <input
							type="text"
							placeholder="<fmt:message key="label.entersurname" />"
							name="user_last_name" class="form-control" required >
					</div>
					<div class="form-group">
						<label for="email"><fmt:message key="label.email" /></label> <input
							type="email" placeholder="<fmt:message key="label.enteremail" />"
							name="user_registration_email" class="form-control" required>
					</div>
					<div class="form-group">

						<label for="password"><fmt:message
								key="label.passwordregister" /></label> <input type="password"
							placeholder="<fmt:message key="label.enterpassword" />"
							name="user_registration_password" class="form-control" required>
					</div>
					<div class="form-group">
						<label for="password"><fmt:message key="label.reppassword" /></label>
						<input type="password"
							placeholder="<fmt:message key="label.reppassword" />"
							name="password-repeat" class="form-control" required >
					</div>
