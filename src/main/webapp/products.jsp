...
<a href="index.jsp"><fmt:message key="label.about" /></a>
			<form action="FrontController" method="post">
				<input type="hidden" id="command" name="command"
					...
					name="pageName" value="products.jsp"> <select id="language"
			...
			<c:if test="${current_user.getUserType().equals('admin')}">
				<a href="admin.jsp"><c:out value="${current_user.getUserName()}" /></a>
				<form action="FrontController" method="get">
			...
			<form action="FrontController" method="get">
				...
				<div class="row row-cols-3 g-3" style="text-align: left">
					...
					<div class="col">
						...
						<div class="input-group mb-3">
							<div class="input-group-prepend">
								<label class="input-group-text" for="selectCategory">Categories</label>
							</div>
					...
					<div class="col">
						...
						<div class="input-group mb-3">
							<div class="input-group-prepend">
								<label class="input-group-text" for="selectColor">Colors</label>
							</div>
	...
	<form action="FrontController" method="get">
		<input type="hidden" id="command" name="command" value="GET_PRODUCTS_AND_PROPERTIES_LIST">
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination justify-content-center">
			<li class="page-item"><c:if test="${currentPageProducts != 1}">
					<a class="page-link"
						href="FrontController?command=GET_PRODUCTS_AND_PROPERTIES_LIST&page=${currentPageProducts - 1}">Previous</a>
				</c:if></li>
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link"
							href="FrontController?command=GET_PRODUCTS_AND_PROPERTIES_LIST&page=${i}">${i}</a>
						</li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<li class="page-item"><c:if
					...
					<a class="page-link"
						href="FrontController?command=GET_PRODUCTS_AND_PROPERTIES_LIST&page=${currentPageProducts + 1}">Next</a>
				</c:if></li>
		</ul>
	</nav>
			...
			<div class="col">
				<div class="card h-100">
					...
					<div class="card-body d-flex flex-column">
						...
						<c:forEach items="${categoryList}" var="category">
							<c:if
								test="${category.getCategoryId() == product.getCategoryId()}">
								<p class="card-text"><fmt:message
						key="label.category" />:
									${category.getCategoryTitle()}</p>
							</c:if>
						</c:forEach>
						<c:forEach items="${colorList}" var="color">
							<c:if
								test="${color.getColorId() == product.getColorId()}">
								<p class="card-text"><fmt:message
						key="label.color" />:
									${color.getColorName()}</p>
							</c:if>
						</c:forEach>
					...
					<div class="card-footer bg-transparent border-success border-0">
						<c:if test="${!current_user.getUserType().equals('admin')}">
							...
							<form action="FrontController" method="get">
								<input type="hidden" id="command" name="command"
									value="ADD_TO_CART"> <input type="hidden"
						...
						<c:if test="${current_user.getUserType().equals('admin')}">
							<form style="margin-top: 1em;" action="FrontController"
								method="get">
								<input type="hidden" id="command" name="command"
							...
							<form style="margin-top: 1em;" action="FrontController"
								method="post">
								<input type="hidden" id="command" name="command"
									value="DELETE_PRODUCT"> <input type="hidden"
									id="productId" name="productId"
									value="${product.getProductId()}">
