		...
		<a href="index.jsp"><fmt:message key="label.about" /></a>
			<form action="FrontController" method="post">
				<input type="hidden" id="command" name="command"
					...
					name="pageName" value="order_items.jsp"> <select
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<li class="page-item"><c:if
								...
								<a class="page-link"
									href="FrontController?command=GET_ALL_ORDER_ITEMS&page=${currentPageAllItems - 1}&orderId=${thisOrderId}">Previous</a>
							</c:if></li>
							...
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="FrontController?command=GET_ALL_ORDER_ITEMS&page=${i}&orderId=${thisOrderId}">${i}</a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<li class="page-item"><c:if
								...
								<a class="page-link"
									href="FrontController?command=GET_ALL_ORDER_ITEMS&page=${currentPageAllItems + 1}&orderId=${thisOrderId}">Next</a>
							</c:if></li>
					</ul>
				</nav>

				<c:if test="${current_user.getUserType().equals('admin')}">
					<a class="btn btn-secondary" href="orders.jsp" role="button"><fmt:message
							key="label.goback" /></a>
				</c:if>
				<c:if test="${current_user.getUserType().equals('normal')}">
					<a class="btn btn-secondary" href="orders_user.jsp" role="button"><fmt:message
							key="label.goback" /></a>
				</c:if>
			</div>
		</div>
