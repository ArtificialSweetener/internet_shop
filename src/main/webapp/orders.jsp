...
<c:if test="${current_user == null}">
	<c:set var="message"
		value="You are not logged in. Please, login as an admin first to get access to this page."
		scope="session" />
		...
		<a href="index.jsp"><fmt:message key="label.about" /></a>
			<form action="FrontController" method="get">
				<input type="hidden" id="command" name="command"
					...
					name="pageName" value="orders.jsp"> <select id="language"
		...
		<div class="card-body">
			<div class="row">
				<h1 class="text-left pt-3 pb-3">
					<fmt:message key="label.welcometoorders" />
				</h1>
				...
				<table class="table table-striped table-hover">
					<thead>
						<tr>
							...
							<th scope="col"><fmt:message key="label.changeorderstatus" /></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${allOrdersList}" var="order">
							<tr>
								...
								<td><form action="FrontController" method="post">
										<input type="hidden" id="command" name="command"
											value="CHANGE_ORDER_STATUS"> <input type="hidden"
											id="orderId" name="orderId" value="${order.getOrderId()}">
										<select id="status" name="status"
											onchange="this.form.submit()">
											<option selected><fmt:message
													key="label.changeorderstatus" />:
											</option>
											<option value="registered"><fmt:message
													key="label.registered" /></option>
											<option value="paid"><fmt:message key="label.paid" /></option>
											<option value="cancelled"><fmt:message
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<li class="page-item"><c:if
								test="${currentPageAllOrders != 1}">
								<a class="page-link"
									href="FrontController?command=GET_ALL_ORDERS_LIST&page=${currentPageAllOrders - 1}">Previous</a>
							</c:if></li>


<!--  <li class="page-item"></li>-->
						<c:forEach begin="1" end="${noOfPagesAllOrders}" var="i">
							<c:choose>
									<c:when test="${currentPageAllOrders eq i}">
									...
									<li class="page-item">
										<a class="page-link"
											href="FrontController?command=GET_ALL_ORDERS_LIST&page=${i}">${i}</a>
											</li>
									</c:otherwise>
								</c:choose>
						</c:forEach>
						...
						<li class="page-item">
						<c:if test="${currentPageAllOrders lt noOfPagesAllOrders}">
								<a class="page-link"
									href="FrontController?command=GET_ALL_ORDERS_LIST&page=${currentPageAllOrders + 1}">Next</a>
							</c:if></li>
					</ul>
				</nav>
